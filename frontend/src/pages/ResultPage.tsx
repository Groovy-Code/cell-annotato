...
export default function ResultPage() {
  ...
  const selectedCells = useMemo(
    () => _allCellsCache.filter((c) => selectedIds.has(c.cell_id)),
    [selectedIds],
  );

  // 主图表 option（静态 series + 交互组件）
  const mainOption: EChartsOption = useMemo(() => {
    ...
    return {
      ...
      legend: {
        data: legendData, bottom: 8, type: "scroll" as const,
      ...
      tooltip: {
        ...
        backgroundColor: "#fff", borderColor: "rgba(11,11,11,0.10)", borderWidth: 1, padding: [12, 16],
        ...
        formatter: (params: unknown) => {
          ...
          const nm = (p.seriesName ?? "").startsWith("» ") ? (p.seriesName ?? "").slice(2) : p.seriesName;
  ...
  useEffect(() => {
    ...
    chart.on("mouseover", (params: unknown) => {
      ...
      if (p.data?.length) {
        ...
        setHoveredCell({
          ...
          label: rawName.startsWith("» ") ? rawName.slice(2) : rawName,
    ...
    chart.setOption(mainOption, true);
  ...
  useEffect(() => {
    return () => { chartInstance.current?.dispose(); chartInstance.current = null; chartReady.current = false; };
  }, []);
    ...
    <div className="result-page">
      <header className="result-topbar">
        ...
        <div className="topbar-tools">
          ...
          {selectedIds.size > 0 && <button className="btn-tool" onClick={clearSelection}>✕ 清除 ({selectedIds.size})</button>}
        </div>
        <div className="topbar-spacer" />
        <button className={selectedIds.size > 0 ? "btn-primary" : "btn-disabled"} disabled={selectedIds.size === 0}>
          修改注释
        </button>
      </header>
      ...
      <div className="result-body">
        ...
        <aside className="side-panel">
          ...
          <section className="panel-block">
            <button className={selectedIds.size > 0 ? "btn-primary btn-block" : "btn-disabled btn-block"} disabled={selectedIds.size === 0}>修改注释</button>
